...
function startProduct(){
    ...
    let datasetClasification = listProduct.dataset.clasification;

    let urlProperty = location.search.match(/(\?[a-z]*)/i);
    let urlValue = location.search.match(/((([a-zA-Z0-9])*)$)/i);
    if(datasetClasification === "search"){
        Product.requestProduct(("get/" + urlValue[0]), false);
        setTimeout(Product.stopLoader, 1000);
    }else if(datasetClasification !== "general"){
        Product.requestProduct(("clasification/" + datasetClasification), true);
        setTimeout(Product.stopLoader, 1000);
    }else{
        callProductGeneral();
    }
...
//Call Products General
function callProductGeneral(){
    Product.requestProduct("clasification/general", true);
    if($index >= 2){
        Product.stopLoader();
    }else{
        setTimeout(()=>{
...
function showAsProduct(){
    ...
    let svgMosaic = btn_mosaic.childNodes[0];
    ...
    btnSearch.onclick = async function(e){
        ...
        if(!(input.value === "general")){
            ...
            await Product.requestProduct("clasification/general", true);
        }
        switch(list_product.dataset.showas){
            case "column": showAsProductColumn();
            break;
...
function showAsProductMosaic(){
    debugger;
    let svgColumn = document.querySelector("#container-product > #container-opt-show-product > form > #show-as-product > #column svg");
    let svgMosaic = document.querySelector("#container-product > #container-opt-show-product > form > #show-as-product > #mosaico svg");
...
function showAsProductColumn(){
    let btn_column = document.querySelector("#container-product > #container-opt-show-product > form > #show-as-product > #column");
    let btn_mosaic = document.querySelector("#container-product > #container-opt-show-product > form > #show-as-product > #mosaico");
    let svgColumn = btn_column.childNodes[0];
